var Ghost = function(game, key, name, startPos, startDir) {
    ...
    switch (this.name) {
        ...
        case "blue":
            offsetGhost = 12;
            //blue starts outside of spawn, so set At_HOME to false
...
Ghost.prototype = {
    update: function() {
        ...
        if (this.isAttacking && (this.mode === this.CHASE)) {
    ...
    move: function(dir) {
        ...
        if (this.mode === this.RANDOM) {
            ...
            if (this.name === "blue" && this.game.numDots < 20) {
                speed = this.cruiseElroySpeed;
                this.mode = this.CHASE;   
            }
        }
