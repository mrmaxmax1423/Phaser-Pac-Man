...
Pacman.prototype.eatDot = function(pacman, dot) {   //when pacman eats a dot
    dot.kill();     //remove dot
    this.game.score ++;         //add score
    this.game.numDots --;
    if (this.game.dots.total === 0) //if no more dots left
    {
        this.game.dots.callAll('revive');       //respawn dots
        this.game.pills.callAll('revive');      //respawn pills
    }
};

Pacman.prototype.eatPill = function(pacman, pill) {     //when pacman eats a pill
    pill.kill();    //remove pill
    
    this.game.score += 10;     //add 10 points
    this.game.numPills --; 
    this.game.enterFrightenedMode();        //ghosts enter blue
};
